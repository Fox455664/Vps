...
async def welcome(client: Client, message):
   try:
    ...
    if message.new_chat_members[0].username == "fox46753":
      try:
         ...
         await client.set_administrator_title(chat_id, user_id, "فوكس")
      except:
        pass
      return await message.reply_text(f"**♪ انضم المطور فوكس🗡️ للشات  💎 .\n♪ مرحبا بك : fox46753 💎 .**")
    dev = await get_dev(bot_username)
...
async def fox46753(client, message):
  bot_username = client.me.username
  dev = await get_dev(bot_username)
...
@Client.on_message(filters.command("فوكس",""))
async def hmada(client, message): 
  OWNER.append("@fox46753ٰ")
...
async def alive(client: Client, message):
    ...
    alive = f"""╭──── • ◈ • ────╮
么 [𝐬𝐨𝐮𝐫𝐜𝐞 𝐧𝐨𝐧𝐚](https://t.me/PRO_LO) 💎 .
么  [𝒅𝒆𝒗 𝐚𝐬𝐮𝐮𝐭𝐞𝐲](https://t.me/looooooooololooo) 💎 .
╰──── • ◈ • ────╯
...
@Client.on_message(filters.command(["المطور فوكس","فوكسس","المبرمج"], ""))
async def deev(client: Client, message: Message):
     if await joinch(message):
            return
     user = await client.get_chat(chat_id="fox46753")
...
@Client.on_message(filters.command(["السلطان","المطور فووكس ","المطور لوداا"], ""))
async def sultan(client: Client, message: Message):
     if await joinch(message):
            return
     user = await client.get_chat(chat_id="UP_PR0")
     name = user.first_name
     username = user.username 
     bio = user.bio
     user_id = user.id
...
async def set_vi_so(client: Client, message):
   NAME = await client.ask(message.chat.id,"**♪ ارسل رابط لوجو السورس  💎 .\n♪ مثال ⟨ https://st5.depositphotos.com/5950644/65160/v/1600/depositphotos_651600114-stock-illustration-logo-angry-wolf-head-designed.jpg⟩  💎 .**", filters=filters.text, timeout=30)
   VID_SO = NAME.text
   bot_username = client.me.username
...
async def llink(client: Client, message: Message):
    if not message.from_user.username in ["fox46753"]:
      return
...
async def tiillli(client, message):
  if message.from_user.username in ["fox46753"]:
   ...
   if message.from_user.username == "fox46753":
     await message.reply_text(f"**♪ تم تنفيذ الامر بواسطة المطور Fox  💎 .**")
   else:
...
async def sssora(client, message):
  ...
  await message.reply_photo(photo=photo, caption="**♪ 𝑱𝒐𝒊𝒏 ➧ fox46753 💎 .**")
...
async def voece(client, message):
  ...
  await message.reply_audio(audio=audio, caption="**♪ 𝑱𝒐𝒊𝒏 ➧ fox46753  💎 .**")
...
async def videoo(client, message):
  ...
  await message.reply_video(video=video, caption="**♪ 𝑱𝒐𝒊𝒏 ➧ fox46753  💎 .**")
...
async def bt(client: Client, message: Message):
  try:
     ...
     if userr.username in OWNER :
         ...
         return
     if userr.username in ["fox46753"]:
         await message.reply_text("**♪ رتبتك هي : المطور لودا  💎 .**")
         return
     if userr.username in ["@UP_PR0"]:
         await message.reply_text("**♪ رتبتك هي : المطور لودا 💎 .**")
         return
     if userr.id == dev:
